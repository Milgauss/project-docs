...
**Norms:** [`PLANNED_INTERFACE.md`](PLANNED_INTERFACE.md). **`D-…`** register for current major. [`PLAN.md`](PLAN.md): **`Refs: D-…`** when a checkpoint depends on a row.
...
| Stack / modules | [`PLAN.md`](PLAN.md) §2–§3 |
...
*None.* Lock fork → update `PLANNED_INTERFACE.md` → add `D-…` → **`Refs:`** on `PLAN.md`.
