"""``API_SPEND_OPENAI_RAW_RESPONSE_PATH`` debug output."""
...
from api_spend.http_raw_dump import OPENAI_RAW_RESPONSE_ENV
...
def test_fetch_costs_writes_raw_envelope_to_file(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    raw_path = tmp_path / "oai_raw.json"
    monkeypatch.setenv(OPENAI_RAW_RESPONSE_ENV, str(raw_path))
    ...
    assert "organization/costs" in outer["url"]
...
def test_raw_dump_skipped_without_env(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    monkeypatch.delenv(OPENAI_RAW_RESPONSE_ENV, raising=False)
